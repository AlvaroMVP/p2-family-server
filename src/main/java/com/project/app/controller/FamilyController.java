...
public class FamilyController {
  ...
  */
  ...
  public Mono<ResponseEntity<Map<String, Object>>> 
      create(@Valid @RequestBody Mono<Family> familyMono) {
  ...
  */
  ...
  public Mono<ResponseEntity<Void>> eliminar(@PathVariable String id) {
    return familyService.findById(id).flatMap(p -> {
      return familyService.delete(p).then(Mono.just(
  ...
  *
  * @param numberDocument in the numberDocument
  * @return the mono
  */
  ...
  */
  ...
  public Mono<ResponseEntity<Family>> findByfullname(@PathVariable String fullname) {
    return familyService.findByfullname(fullname)
  .map(p -> ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON_UTF8).body(p))
  .defaultIfEmpty(ResponseEntity.notFound().build());
  }
